...
karpenter.sh/nodepool: {{ include "druid.component.name" (list $component $ctx) }}-nodepool
{{- end -}}
...
- name: DRUID_AZURE_STORAGE_ACCOUNT
  value: {{ .Values.azure.storageAccount | quote }}
- name: DRUID_AZURE_INDEXLOGS_CONTAINER
  value: {{ .Values.azure.indexLogsContainer | quote }}
- name: DRUID_AZURE_DEEPSTORAGE_CONTAINER
  value: {{ .Values.azure.deepStorageContainer | quote }}
- name: DRUID_AZURE_MSQ_CONTAINER
  value: {{ .Values.azure.msqContainer | quote }}
  ...
  template:
    metadata:
      labels:
        ...
        karpenter.sh/nodepool: {{ $nodeName }}-nodepool
    spec:
      requirements:
        - key: "karpenter.sh/nodepool"
          operator: In
          values: ["{{ $nodeName }}-nodepool"]
...
karpenter.sh/nodepool: {{ include "druid.name" . }}-druid-task-base-nodepool
{{- end -}}
