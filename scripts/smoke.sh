...
if [[ -n "$apps_json" ]]; then
  ...
  if [[ "$total" -gt 0 && "$synced" == "$total" && "$healthy" == "$total" ]]; then
    pass "all $total Applications Synced + Healthy"
  else
    bad=$(echo "$apps_json" | jq -r '.items[] | select(.status.sync.status!="Synced" or .status.health.status!="Healthy") | "\(.metadata.name) (sync=\(.status.sync.status) health=\(.status.health.status))"' | head -5 | tr '\n' '; ')
    fail "argocd applications" "$synced/$total Synced, $healthy/$total Healthy — bad: $bad"
  fi
else
