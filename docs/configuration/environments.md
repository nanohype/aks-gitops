...
|-------------|---------|--------------|--------------|
| dev | Development and testing | dev-aks | Audit |
| staging | Pre-production validation | staging-aks | Enforce |
| production | Live workloads | production-aks | Enforce |
...
The `environment` label on the cluster secret is what ApplicationSets use to select the correct values files or overlay paths. The `provider` field identifies this as an AKS cluster in the multi-cloud strategy.
...
| Backup Container | none | velero (aksstagingbackups) | velero (aksproductionbackups) |
