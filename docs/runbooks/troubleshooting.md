...
4. For Azure-specific issues, verify the Workload Identity binding to the Storage Blob Data Contributor role on the backup storage account.
...
2. Check the federated credential maps the service account to the managed identity:
   ```bash
   az identity federated-credential list --identity-name <name> --resource-group <rg>
   ```

3. The service account must have:
   - Label: `azure.workload.identity/use: "true"`
   - Annotation: `azure.workload.identity/client-id: <client-id>`

4. Pods must have the matching label for the webhook to project the token:
